    ...
    try:
        ...
        result = {
            ...
            "key_points": {
                ...
                "debt_to_equity": dte if dte is not None else "N/A",
                "revenue_growth": f"{rev_growth}%" if rev_growth is not None else "N/A",
                "earnings_growth": f"{profit_growth}%" if profit_growth is not None else "N/A",
                "profit_margins": f"{safe_round(float(profit_margins) * 100)}%" if profit_margins and profit_margins != "N/A" else (f"{opm}%" if opm is not None else "N/A"),
